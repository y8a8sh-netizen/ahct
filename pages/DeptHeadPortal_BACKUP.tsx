...
const DeptHeadPortal: React.FC<DeptHeadPortalProps> = ({ data }) => {
  ...
  const filteredCommittees = useMemo(() => {
      return data.committees.filter(comm => {
          ...
          // البحث في رقم اللجنة أو اسم المقرر أو رمز المقرر
  ...
  const statistics = useMemo(() => {
    ...
    // حساب الامتحانات حسب التاريخ
    const examsByDate: Record<string, number> = {};
    filtered.forEach(comm => {
      const exam = data.exams.find(e => e.courseCode === comm.examCode);
      if (exam?.date) {
        examsByDate[exam.date] = (examsByDate[exam.date] || 0) + 1;
      }
    });
...
const CommitteesTab: React.FC<CommitteesTabProps> = ({
  ...
  // عرض صفحة جداول المراقبين إذا تم تفعيلها (بعد جميع الـ hooks)
  if (showProctorSchedules) {
    return (
      <div>
        <button 
          onClick={() => setShowProctorSchedules(false)}
          className="mb-4 bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 transition-all flex items-center gap-2"
        >
          ← العودة إلى لوحة اللجان
        </button>
  ...
  // -------------------------------------------------------------
  // NATIVE PRINTING LOGIC (POP-UP HTML INJECTION)
  // -------------------------------------------------------------
  const handlePrintAttendanceSheets = () => {
      if (filteredCommittees.length === 0) {
          alert("لا توجد لجان لطباعتها. يرجى تعديل الفلاتر.");
          return;
      }
      ...
      // Configuration for manual pagination
      const ROWS_PER_PAGE = 18; // Safe number of students per page to fit A4 with headers/footers

      // Construct all pages HTML
      let pagesHtml = '';
      ...
      filteredCommittees.forEach((comm) => {
          ...
          // Calculate total pages for this specific committee
          const totalPages = Math.ceil(Math.max(committeeStudents.length, 1) / ROWS_PER_PAGE);
          ...
          for (let page = 1; page <= totalPages; page++) {
              ...
              // Pad empty rows ONLY if it's the last page to maintain layout consistency
              // Or to make sure the footer is pushed down appropriately
              const rowsOnThisPage = pageStudents.length;
              if (rowsOnThisPage < ROWS_PER_PAGE) {
                  const emptyRowsNeeded = ROWS_PER_PAGE - rowsOnThisPage;
                  for (let i = 0; i < emptyRowsNeeded; i++) {
      ...
      const fullHtml = `
        ...
        <html dir="rtl">
        <head>
            <title>كشوف التحضير - ${selectedDept || 'الكلية التقنية'}</title>
            <link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;700&display=swap" rel="stylesheet">
            <style>
                @page { size: A4; margin: 0; } /* Zero margin here, handled in .page */
                body { font-family: 'Tajawal', sans-serif; margin: 0; padding: 0; -webkit-print-color-adjust: exact; background: #fff; }
                
                /* Container for each page */
                .page { 
                    width: 210mm;
                    height: 296mm; /* Force A4 Height */
                    padding: 10mm 10mm;
                    box-sizing: border-box;
                    page-break-after: always; 
                    break-after: page;
      ...
      <div className="bg-gradient-to-r from-tvtc-green to-green-600 text-white p-6 rounded-xl shadow-lg">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => setShowProctorSchedules(true)}
              className="bg-white text-tvtc-green px-5 py-2.5 rounded-lg flex items-center gap-2 hover:bg-green-50 shadow-md transition-all font-semibold"
            >
              <Users size={18} /> جداول المراقبين
            </button>
            <button 
              onClick={handlePrintAttendanceSheets}
              disabled={filteredCommittees.length === 0}
              className="bg-gray-800 text-white px-5 py-2.5 rounded-lg flex items-center gap-2 hover:bg-gray-900 shadow-md transition-all font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Printer size={18} /> طباعة الكشوف ({filteredCommittees.length})
            </button>
          </div>
        </div>
      </div>
